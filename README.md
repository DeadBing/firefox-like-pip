...
- Iframe embeds (Alloha-style players, many “запасной плеер” hosts) clone the video through a same-tab WebRTC bridge. The top frame must send the WebRTC answer *before* waiting for `ontrack`, or Chrome never delivers the track and the empty window closes. The answer still has to include ICE candidates (and trickle the rest); an answer without candidates leaves a black Document PiP that never paints. If the site’s file is cross-origin without CORS, `captureStream()` is a black frame and the extension uses native video PiP.
