...
- Iframe embeds (Alloha-style players, many “запасной плеер” hosts, YouTube embeds) clone the video through a same-tab WebRTC bridge. Same-origin iframes skip that encode and capture the element from the top frame. If `captureStream()` only has the small on-page box (a 360px iframe, a compact YouTube player), the clone is painted from the decoded `videoWidth` × `videoHeight` instead. Cross-origin clones still go through WebRTC (VP9, full resolution, high bitrate); they will not be bit-identical to the page. If the site’s file is cross-origin without CORS, `captureStream()` is a black frame and the extension uses native video PiP.
